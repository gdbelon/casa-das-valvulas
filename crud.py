...
def cadastrar_produto(nome, qtd, marca):
    ...
    try:
        ...
        cur.execute(
            'INSERT INTO produtos (nome_valvula,qtd,marca) VALUES (%s,%s,%s) RETURNING *',(nome, qtd, marca)
        )
        row_2 = cur.fetchone()
        conn.commit()
...
def atualizar_nome_valvula(id, nome):
    ...
    try:
        ...
        cur.execute(
            'UPDATE produtos SET nome_valvula = %s WHERE id = %s RETURNING *',(nome, id)
        )
        row_2 = cur.fetchone()
        conn.commit()
        return {"mensagem": row_2}
...
def atualizar_qtd(id, qtd):
    ...
    try:
        cur.execute(
            'UPDATE produtos SET qtd = %s WHERE id = %s RETURNING *',(qtd, id)
            )
        row = cur.fetchone()
        conn.commit()
        return {"mensagem": row}
